...
import { parseExpects, evaluateExpects } from './expect.js';
import { friendlyErrors } from './errors.js';
...
export class LiveSession {
    ...
    get instrCount() { return this.parsed?.instrs?.length ?? 0; }
    get allCells() { return this.cells; }
    rebuild(cells) {
        this.cells = cells;
        this.outputs.clear();
        ...
        const parsed = new Parser().parse(concat);
        this.parsed = parsed;
    ...
    userLineToParserLine(cellId, userLine) {
    ...
    runToLine(cellId, line) {
        if (!this.built)
            return this.errorResult('No program built yet');
        if (this.needsRestart)
            return this.errorResult('Machine needs restart', 'restart-needed');
        // Convert user line number to parser's lineNum
        const parserLine = this.userLineToParserLine(cellId, line);
        if (parserLine === null)
            return this.errorResult(`Line ${line} not found in cell ${cellId}`);
        ...
        const outStart = this.ex.output?.length ?? 0;
        let reason = 'end';
        ...
        while (steps < stepsLimit) {
            try {
                ...
                runtimeError = e.message || 'Unknown execution error';
                reason = 'error';
                break;
            }
            steps++;
            ...
            if (this.breakpoints.has(this.cpu.ip)) {
                reason = 'breakpoint';
                break;
            }
        }
    ...
    run(targetCellId, mode) {
        ...
        while (steps < stepsLimit) {
            ...
            if (ip >= this.parsed.instrs.length) {
                reason = 'end';
                break;
            }
    ...
    get currentExecCount() {
        return this.globalExecCount;
    }
    errorResult(msg, reason = 'error') {
    ...
    step() {
        ...
        try {
            ...
            return { reason: 'error', error: e.message || 'Unknown', steps: 0, output: '', regDiff: {}, halted: false, expectResults: [], allPassed: true };
        }
        const after = this.snapshotRegs();
        const ip = this.cpu.ip;
    ...
    reset(clearBps = false) {
        this.resetMachine(clearBps);
        this.outputs.clear();
    }
    snapshotRegs() {
        const r = {};
        for (const k of REG_LIST) {
            r[k] = this.cpu.getReg(k) ?? 0;
        }
    ...
    evalCtx() {
        const cpu = this.cpu;
        return {
            getReg: (n) => cpu.getReg(n),
            getFlag: (n) => { const f = cpu.flags[n.toUpperCase()]; return f !== undefined ? f : null; },
            memReadByte: (linear) => cpu.memRead(linear & 0xfffff, 8),
            getScreenChar: (row, col) => {
    ...
    getParseErrors() {
        return (this.parsed?.errors || []).map((e) => ({
            cellId: e.lineNum ? this.lineOwner[e.lineNum - 1] ?? null : null,
            line: e.lineNum ?? null,
            message: e.message
        }));
    }
    toggleBreakpoint(cellId, lineInCell) {
        const key = cellId + ':' + lineInCell;
        if (this.bpSource.has(key)) {
            this.bpSource.delete(key);
            this.resyncBreakpoints();
            return false;
        }
    ...
    resyncBreakpoints(parserLine) {
        ...
        for (const key of this.bpSource) {
            ...
            if (parserLine !== undefined) {
                ...
                idx = this.parsed.instrs.findIndex((ins) => ins.lineNum === parserLine);
            }
            else {
                // Original behavior: match by absolute line number
                idx = this.parsed.instrs.findIndex((ins) => ins.lineNum === abs);
            }
            if (idx >= 0)
                this.breakpoints.add(idx);
        }
    }
    getBreakpointLines(cellId) {
        const s = new Set();
        for (const key of this.bpSource) {
            const [cid, ln] = key.split(':');
            if (cid === cellId)
    ...
    getCellLocalLine(cellId, globalLine) {
    ...
    getVideoEvents() {
        return this.ex.video ?? [];
    }
    getOutput(cellId) { return this.outputs.get(cellId); }
    getAllOutputs() { return new Map(this.outputs); }
    /** Clear output for a specific cell. */
    clearOutput(cellId) { this.outputs.delete(cellId); }
    /** Clear all outputs. */
    clearAllOutputs() { this.outputs.clear(); }
    memHex(linear, rows = 8) {
        const out = [];
        for (let r = 0; r < rows; r++) {
            const addr = linear + r * 16;
            const bytes = [];
            for (let b = 0; b < 16; b++)
                bytes.push(this.cpu.memRead(addr + b, 8));
            out.push({ addr, bytes });
        }
        return out;
    }
    stackView(depth = 8) {
        ...
        for (let i = 0; i < depth; i++)
            words.push(this.cpu.memRead(linear + i * 2, 16));
