...
function matchError(text, patterns) {
...
export function friendlyParse(text, line) {
    const txt = text.trim();
    const nas = matchError(txt, NASM_PATTERNS);
    if (nas)
        return { line, original: txt, friendly: nas.friendly, hint: nas.hint, message: nas.friendly };
    const run = matchError(txt, RUNTIME_PATTERNS);
    if (run)
        return { line, original: txt, friendly: run.friendly, hint: run.hint, message: run.friendly };
...
export function friendlyErrors(errors) {
